...
PyTypeObject Idle_Type = {
    ...
    .tp_doc = "Idle(loop, callback[, data=None, priority=0])",
    .tp_new = Idle_tp_new,
};
...
Watcher *
Idle_New(Loop *loop, PyObject *args, PyObject *kwargs)
{
    ...
    if (PyArg_ParseTupleAndKeywords(args, kwargs, "O|Oi:idle", kwlist,
            &callback, &data, &priority) &&
        (self = __Idle_New(&Idle_Type)) &&
