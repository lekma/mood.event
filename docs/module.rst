:py:mod:`mood.event` --- Python libev interface
===============================================

.. py:module:: mood.event
    :platform: POSIX
    :synopsis: Python libev interface


.. py:function:: loop([flags=EVFLAG_AUTO, callback=None, data=None, io_interval=0.0, timeout_interval=0.0])

    :rtype: :py:class:`Loop`

    This will instanciate the *default loop* if it hasn't been created yet and
    return it. If the *default loop* was already initialized this simply returns
    it (and ignores the arguments).

    The :dfn:`default loop` is the only loop that can handle :py:class:`Child`
    watchers, and to do this, it always registers a handler for ``SIGCHLD``. If
    this is a problem for your application you can either instanciate a
    :py:class:`Loop` which doesn't do that, or you can simply overwrite the
    ``SIGCHLD`` signal handler.

    See :py:class:`Loop` for details about the arguments.

    .. note::

        If you don't know what loop to use, use the one returned from this
        function.


.. py:function:: time

    :rtype: float

    Returns the current time as libev would use it.

    .. note::

        The :py:meth:`~Loop.now` method is usually faster and also often returns
        the timestamp you actually want to know.


.. py:function:: sleep(interval)

    :param float interval: interval in seconds.

    Sleep for the given *interval*. The current thread will be blocked until
    either it is interrupted or the given time interval has passed
    (approximately - it might return a bit earlier even if not interrupted).
    Returns immediately if *interval* <= ``0.0``.

    .. note::

        The range of *interval* is limited - libev only guarantees to work with
        sleep times of up to one day (*interval* <= ``86400.0``).


.. py:function:: abi_version

    :rtype: tuple(int, int)

    Returns a tuple of major, minor version numbers. These numbers represent the
    libev ABI version that this module is running.

    .. note::

        This is not the same as libev version (although it might coincide).


.. py:function:: supported_backends

    :rtype: int

    Returns the set of all :ref:`backends` compiled into this binary of libev
    (independent of their availability on the system you are running on).


.. py:function:: recommended_backends

    :rtype: int

    Returns the set of all :ref:`backends` compiled into this binary of libev
    and also recommended for this platform, meaning it will work for most file
    descriptor types. This set is often smaller than the one returned by
    :py:func:`supported_backends`, as for example ``kqueue`` is broken on most
    BSDs and will not be auto-detected unless you explicitly request it.
    This is the set of backends that libev will probe for if you specify no
    backends explicitly.


.. py:function:: embeddable_backends

    :rtype: int

    Returns the set of :ref:`backends` that are embeddable in other event loops.
    This value is platform-specific but can include backends not available on
    the current system. To find which embeddable backends might be supported on
    the current system, you would need to look at:

        .. code-block:: python

            embeddable_backends() & supported_backends()

    likewise for recommended ones:

        .. code-block:: python

            embeddable_backends() & recommended_backends()

    See :py:class:`Embed` watchers for more information about embedding loops.


.. py:function:: feed_signal(signum)

    :param int signum: signal number to feed libev.

    This function can be used to "simulate" a signal receive. It is completely
    safe to call this function at any time, from any context, including signal
    handlers or random threads. Its main use is to customise signal handling in
    your process, especially in the presence of threads.

    For example, using :py:func:`~signal.pthread_sigmask`, you could ignore
    signals by default in all threads (and specify :py:data:`EVFLAG_NOSIGMASK`
    when creating any loops), and in one thread, wait for signals, then
    "deliver" them to libev by calling :py:func:`feed_signal`.


.. py:decorator:: fatal

    A callback using this decorator will stop the loop if an unhandled exception
    happens during its execution.


.. py:data:: __version__

    mood.event's version.


.. py:exception:: Error

    Raised when an error specific to mood.event happens.


Objects
-------

.. toctree::
    :maxdepth: 2
    :titlesonly:

    Loop
    Watcher
